...
# ---------------------------------------------------------------------------
# Refunds — record as a negative booking on the sold Sofi trip
# ---------------------------------------------------------------------------
async def push_refund_to_sofi_as_booking(itn: dict, refund: dict) -> dict:
    """Register an executed refund as a NEGATIVE booking row on the trip's
    Sofi record. Sofi doesn't have a first-class refund concept for the
    reservas table, but a negative-amount booking (title prefixed with
    "REEMBOLSO —") is picked up by their totals aggregation and shows up
    clearly in the timeline.

    Falls back to a positive-amount booking with a "[REEMBOLSO]" tag if
    the initial POST is rejected by MySQL (defensive — Sofi's schema
    accepts decimals, so negatives should be fine).

    Returns { ok, sofi_booking_id?, url? } / { ok: False, error }.
    """
    if not GESTION_USER or not GESTION_PASS:
        return {"ok": False, "error": "Sofi credentials not configured"}
    trip_id = itn.get("sofi_trip_id")
    if not trip_id:
        return {"ok": False, "error": "El viaje aún no tiene sofi_trip_id"}
    qty = int(itn.get("num_travelers") or 1) or 1
    amount = float(refund.get("amount_eur") or 0)
    if amount <= 0:
        return {"ok": False, "error": "Importe de reembolso inválido"}
    reason = (refund.get("reason") or "").strip() or "sin motivo"

    def build_booking(negative: bool) -> dict:
        signed = -amount if negative else amount
        prefix = "REEMBOLSO —" if negative else "[REEMBOLSO] —"
        return {
            "kind": "refund",
            "trip_id": int(trip_id),
            "service_name": f"{prefix} {reason}"[:120],
            "city": "",
            "quantity": qty,
            "date_entry": (refund.get("decided_at") or refund.get("requested_at") or "")[:10],
            "date_exit": None,
            "room": None,
            "invoice_excl": round(signed, 2),
            "invoice_incl": round(signed, 2),
            "price_total": round(signed, 2),
            "notes": (
                f"[REEMBOLSO POST-VENTA] Aprobado por {refund.get('approved_by') or '?'} "
                f"el {(refund.get('decided_at') or '')[:10]}. "
                f"Origen: pago {refund.get('payment_id') or '?'}. "
                f"PayPal refund id: {refund.get('paypal_refund_id') or 'n/a'}. "
                f"Motivo: {reason}."
            )[:500],
            "provider": None,
            "type_radio": 0,
    ...
    async with _BROWSER_SEMAPHORE:
        async with async_playwright() as pw:
            ...
            try:
                ...
                # Fresh booking form → get hidden CSRF
                await page.goto(f"{GESTION_BASE}/reservas/form/3/",
                                wait_until="domcontentloaded", timeout=30000)
                try:
                    await page.wait_for_selector("input[name*='__form_id']",
                ...
                # First try with a NEGATIVE amount (accurate accounting).
                op_cache: dict = {}
                first = await _push_one_booking_fast(
                    page, build_booking(negative=True), hidden, op_cache
                )
                if first.get("ok"):
                    return first
                # If Sofi choked on negatives, retry with a positive amount
                # tagged as [REEMBOLSO]. Refresh hidden fields first in case
                # the CSRF was consumed by the failed attempt.
                await page.goto(f"{GESTION_BASE}/reservas/form/3/",
                                wait_until="domcontentloaded", timeout=30000)
                try:
                    await page.wait_for_selector("input[name*='__form_id']",
                ...
                hidden = await _fetch_booking_form_hidden(page)
                if not hidden:
                    return first  # bubble up the original error
                fallback = await _push_one_booking_fast(
                    page, build_booking(negative=False), hidden, op_cache
                )
                fallback["negative_amount_rejected"] = True
                return fallback
            finally:
                await browser.close()
